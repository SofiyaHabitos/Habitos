...
=end
    ...
    if file1.present?
      ...
      if file1.first == headers
        ...
        if tenant_name.compact.count == 1
              ...
              if tenant.tenant_details.pluck(:meal_time).flatten.uniq.map { |e| e.downcase }.include?(data[7].downcase.strip)
                ...
                if tenant.tenant_details.pluck(:stud_type).include?data[17]
                  ...
                  if tenant.tenant_details.pluck(:meal_restriction_id).flatten.uniq.include?(meal_restriction_id) or Standard.where(tenant_id:tenant.id).pluck(:standard).include?data[3] 
                    Standard.find_or_create_by(standard: data[3],tenant_id: tenant.id)                 
                  
                    enrolment = Enrolment.new(tenant_id: tenant.id, name: data[1], id_number: data[2], standard: data[3], sex: data[4], age_type: data[5], date_of_birth: data[6], meal_time: data[7], from_date: data[8], to_date: data[9], height: data[10], weight: data[11], bmi: data[12], email: data[13], parent_name: data[14], order_source: data[15], meal_restriction_id: meal_restriction_id, food_category_id: food_category_id, stud_type: stud_type,academic_year: data[18], is_active: true, status: "Approved")
                  ...
                  if enrolment.save
                    enrolments << enrolment
                      ...
                      OldEnrolment.create(name:old_enrol.name,age_type: old_enrol.age_type, date_of_birth: old_enrol.date_of_birth, height: old_enrol.height,weight: old_enrol.weight, bmi: old_enrol.bmi, standard: old_enrol.standard , id_number: old_enrol.id_number,food_category_id: old_enrol.food_category_id,enrolment_type: old_enrol.enrolment_type, tenant_id: old_enrol.tenant_id, order_source: old_enrol.order_source, email: old_enrol.email,sex: old_enrol.sex,parent_name: old_enrol.parent_name, meal_time: old_enrol.meal_time,from_date: old_enrol.from_date ,to_date: old_enrol.to_date,meal_restriction_id: old_enrol.meal_restriction_id, comment: old_enrol.comment, is_active: old_enrol.is_active, stud_type: old_enrol.stud_type,status: old_enrol.status, academic_year: old_enrol.academic_year)
                       new_enrol= Enrolment.find_by(id_number: data[2]).update(tenant_id: tenant.id, name: data[1], id_number: data[2], standard: data[3], sex: data[4], age_type: data[5], date_of_birth: data[6], meal_time: data[7], from_date: data[8], to_date: data[9], height: data[10], weight: data[11], bmi: data[12], email: data[13], parent_name: data[14], order_source: data[15], meal_restriction_id: meal_restriction_id, food_category_id: food_category_id, stud_type: data[17], academic_year: data[18],is_active: true, status: "Approved")
                       if new_enrol == true
...
=end
  ...
  def self.Tenant_update_details(params)
    enrolment = Enrolment.where(id: params[:approval_id],status: "Approved", is_active: true)
    saved ,meal_time_error, meal_restriction_error, stud_type_error , invalid_date = [], [], [],[], []

      enrolment.map do |enrol|
        tenant = Tenant.find_by("from_date <= ? AND to_date >= ? AND id =?" ,enrol.from_date, enrol.to_date, enrol.tenant_id)
        if tenant.present?
            tenant_meal_time = tenant.tenant_details.where(meal_time: enrol.meal_time)
            if tenant_meal_time.present?
                meal_restriction = tenant_meal_time.where(meal_restriction: enrol.meal_restriction_id)
                if meal_restriction.present?
                     final_data = meal_restriction.where(stud_type: enrol.stud_type)

                    if final_data.present?
                     if final_data.update(quantity: final_data.first.quantity+1)
                     saved << final_data.count 
                     return_data ="#{saved.first} values Saved Successfully!!" 
                     end

                    else
                      return_data = "Student Type is Not in Contract!!"     
                    end
                else
                return_data = "MealRestriction  is Not in Contract!!"     
                end
                
            else
             return_data = "MealTime is Not in Contract!!"
            end

        else
          return_data = "Contract Date Is Exceed!!"
        end
      end
